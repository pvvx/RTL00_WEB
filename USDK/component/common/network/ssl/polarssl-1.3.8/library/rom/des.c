...
int des_setkey_enc( des_context *ctx, const unsigned char key[DES_KEY_SIZE] )
{
    ...
    {
        memcpy(ctx->enc_key, key, DES_KEY_SIZE);
    }
#endif /* RTL_HW_CRYPTO */
...
int des_setkey_dec( des_context *ctx, const unsigned char key[DES_KEY_SIZE] )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        memcpy(ctx->dec_key, key, DES_KEY_SIZE);
    }
#endif /* RTL_HW_CRYPTO */
    des_setkey( ctx->sk, key );
                      ...
                      const unsigned char key[DES_KEY_SIZE * 2] )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        memcpy(ctx->enc_key, key, DES_KEY_SIZE * 2);
        memcpy(ctx->enc_key + DES_KEY_SIZE * 2, key, DES_KEY_SIZE);
    }
#endif /* RTL_HW_CRYPTO */
                      ...
                      const unsigned char key[DES_KEY_SIZE * 2] )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        memcpy(ctx->dec_key, key, DES_KEY_SIZE * 2);
        memcpy(ctx->dec_key + DES_KEY_SIZE * 2, key, DES_KEY_SIZE);
    }
#endif /* RTL_HW_CRYPTO */
                      ...
                      const unsigned char key[DES_KEY_SIZE * 3] )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        memcpy(ctx->enc_key, key, DES_KEY_SIZE * 3);
    }
#endif /* RTL_HW_CRYPTO */
                      ...
                      const unsigned char key[DES_KEY_SIZE * 3] )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        memcpy(ctx->dec_key, key, DES_KEY_SIZE * 3);
    }
#endif /* RTL_HW_CRYPTO */
                    ...
                    unsigned char *output )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        ...
        if(length > 0)
        {
            ...
            if(mode == DES_DECRYPT)
            {
                memcpy(key_buf_aligned, ctx->dec_key, DES_KEY_SIZE);
                rom_ssl_ram_map.hw_crypto_des_cbc_init(key_buf_aligned, DES_KEY_SIZE);
            ...
            {
                memcpy(key_buf_aligned, ctx->enc_key, DES_KEY_SIZE);
                rom_ssl_ram_map.hw_crypto_des_cbc_init(key_buf_aligned, DES_KEY_SIZE);
                     ...
                     unsigned char *output )
{
    ...
    if(rom_ssl_ram_map.use_hw_crypto_func)
    {
        ...
        if(length > 0)
        {
            ...
            if(mode == DES_DECRYPT)
            {
                memcpy(key_buf_aligned, ctx->dec_key, DES_KEY_SIZE * 3);
                rom_ssl_ram_map.hw_crypto_3des_cbc_init(key_buf_aligned, DES_KEY_SIZE * 3);
            ...
            {
                memcpy(key_buf_aligned, ctx->enc_key, DES_KEY_SIZE * 3);
                rom_ssl_ram_map.hw_crypto_3des_cbc_init(key_buf_aligned, DES_KEY_SIZE * 3);
